...
public class UserManagement {
	...
	public ResponsePacket createUser(QueryPacket postData) {
		...
		if (!isValidEmailAddress(username)) {
			...
			logger.trace("A user registration has been rejected because the user " + username + " is not considered a valid email address.");
		...
		try {
			...
			logger.warn("Can't add a new user to the DB. SQL says: " + e.getMessage());
			return new ResponsePacket(-1, "The user registration is failed: maybe the chosen username already exists.");
		}
		
		logger.info("Added a new user to the database, with EMAIL=" + username + ".");
	...
	public ResponsePacket getReputation(QueryPacket postData) {
		...
		if (Tools.isNotValid(username, Tools.USERNAME_REGEX) || Tools.isNotValid(password, Tools.PASSWORD_REGEX)) {// TODO: to be completed
            ...
            logger.warn("The getReputation() was called with wrong parameters.");
		...
		try {
			...
			logger.error("Thrown SQL exception while trying to get the reputation of an user: " + e.getMessage());
            return new ResponsePacket(-1, Messages.DB_SELECT_FAILED);
		}
		
		logger.trace("User with EMAIL=" + username + " has asked for his rep. Result is: " + currentUser.getTV());
	...
	public ResponsePacket changePassword(QueryPacket postData) {
		...
		if (Tools.isNotValid(username, Tools.USERNAME_REGEX) || Tools.isNotValid(oldPassword, Tools.PASSWORD_REGEX)
            ...
            logger.warn("The changePassword() was called with wrong parameters.");
		...
		try {
			...
			logger.error("Thrown SQL exception while trying update the password of an user: " + e.getMessage());
            return new ResponsePacket(-1, Messages.DB_INSERT_FAILED);
		}
		
		logger.trace("Updated password for user with EMAIL=" + username);
