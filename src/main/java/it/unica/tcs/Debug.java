...
public class Debug {
	...
	public ResponsePacket tellStandard(QueryPacket postData) {
        ...
        String compliant = "?pay{;t}.(?ok & ?dispute{t<10; t}.!refund{t<7} & ?abort)";
        String notCompliant = "?pay{;t}.(?ok & ?dispute{t<9; t}.!refund{t<7} & ?abort)";
		
		String firstXml = CTU.tstToXml(compliant);
		String secondXml = CTU.tstToXml(notCompliant);
        
        AppResponse firstMapping = Tools.callApplication(Tools.getCtuPath() + Tools.CTU_PARAM_BUILD_AUTOMATON, firstXml);
        AppResponse firstLabels = Tools.callApplication(Tools.getCtuPath() + Tools.CTU_PARAM_GET_LABELS, firstXml);
        
        AppResponse secondMapping = Tools.callApplication(Tools.getCtuPath() + Tools.CTU_PARAM_BUILD_AUTOMATON, secondXml);
        AppResponse secondLabels = Tools.callApplication(Tools.getCtuPath() + Tools.CTU_PARAM_GET_LABELS, secondXml);
        ...
        try {
	        for (int i=0; i<N; i++) {
	        	...
	        	if (Math.random() < K/N) 
	        		db.insertContract(Tools.hashContract(firstXml, MainApplication.getRand()), firstXml, 27230, 0, DatabaseInterface.CONTRACT_ROLE_LATENT, DatabaseInterface.CONTRACT_LATENT, new Long(MainApplication.getRand()), ComplianceChecker.getContractType(firstXml), firstMapping.getOutput(), firstLabels.getOutput(), 0); 
	        	else
	        		db.insertContract(Tools.hashContract(secondXml, MainApplication.getRand()), firstXml, 27230, 0, DatabaseInterface.CONTRACT_ROLE_LATENT, DatabaseInterface.CONTRACT_LATENT, new Long(MainApplication.getRand()), ComplianceChecker.getContractType(secondXml), secondMapping.getOutput(), secondLabels.getOutput(), 0);
	        }
        }
